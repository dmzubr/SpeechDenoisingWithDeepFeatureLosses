...
from message_handler import DFLDenoisingMessageHandler
...
class SplitterAMQPService:
    def __init__(self, config_file_path):
        ...
        self.__channel = None
        self.__in_queue_name = 'Loyalty.Audio.DenoiserDFL.DFLDenoisingRequest, Loyalty.Audio.DenoiserDFL'
        self.__exchange_name = config['exchange_name']  # 'easy_net_q_rpc'
        self.__amqp_host = config['amqp_host']
        self.__amqp_port = config['amqp_port']
        self.__user_name = config['user_name']
        self.__password = config['password']

        self.__response_object_provider = DFLDenoisingMessageHandler(config_file_path)
    ...
    def __handle_delivery(self, channel, method_frame, header_frame, body):
        ...
        try:
            ...
            res_obj = self.__response_object_provider.get_response_obj(req)
    ...
    def __push_message(self, reply_to_key, correlation_id, res_obj):
        res = res_obj
        body = json.dumps(res)
        msg_type = 'Loyalty.Audio.DenoiserDFL.DFLDenoisingResponse, Loyalty.Audio.DenoiserDFL'
